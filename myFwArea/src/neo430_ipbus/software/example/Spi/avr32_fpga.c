...
void fpga_run ( void )
{
  ...
  // We're going to program the FPGA.
  // In case the FPGA reboots because payload power has just come on,
  // reinitialize the SD card system first. Then check whether the SD card is ready.
  ...
  if ( !sd_mmc_mci_dma_read_open ( AVR32_SD_SLOT, xImgStart, ( void* ) FPGA_ADDR, SFWFS_DATABLOCK_SIZE ) )
  {
    dprintf ( "[FPGA] Failed to open SD card for DMA...\n" );
    return;
  }

  U32 lSectorsRemaining = SFWFS_DATABLOCK_SIZE;
...
int fpga_configure_networking()
{
  ...
  if ( xRARP )
  {
    sendIPconfig[1] |= 0x00020000; //Enable RARP
  }
  ...
  bool sameIPaddress = (retIPconfig[2]==sendIPconfig[2]);
  bool sameOtherConfig = ((retIPconfig[0]==sendIPconfig[0]) && (retIPconfig[1]==sendIPconfig[1]));
  if ( (xRARP && !sameOtherConfig) || (!xRARP && !(sameIPaddress && sameOtherConfig)) )
  {
    //dprintf ( "[Net] Configure networking [ 0x%08lX , 0x%08lX , 0x%08lX]\n", sendIPconfig[2] , sendIPconfig[1] , sendIPconfig[0] );
    //dprintf ( "[Net] IP: %u.%u.%u.%u , MAC: %02uX:%02uX:%02uX:%02uX:%02uX:%02uX\n", xIP[0], xIP[1], xIP[2], xIP[3], xMAC[0], xMAC[1], xMAC[2], xMAC[3], xMAC[4], xMAC[5] );
