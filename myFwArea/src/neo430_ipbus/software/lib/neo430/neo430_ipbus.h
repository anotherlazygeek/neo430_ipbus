...
int spi_ipbus_write_mac ( uint32_t Address , const uint48_t* Data , uint32_t PayloadLength, bool NonIncrementing , bool UseConfigurationSpace )
{
...
int spi_ipbus_read_mac ( uint32_t Address , uint48_t* Reply , uint32_t PayloadLength, bool NonIncrementing , bool UseConfigurationSpace )
{

  uint32_t lTransactionHeader , lExpectedHeader,lReplyHeader;
...

  {
    ...
    for ( ; i != PayloadLength-1; ++i )
    {
      Reply[i] = spi_transfer48 ( z );
    }

    // Get the final word of the reply
    Reply[PayloadLength-1] = spi_transfer48 ( y );
    // End the read half of the transaction
    spi_unselectChip( &NEO430_SPI0, SPI0_MEM_SEL );
  }
